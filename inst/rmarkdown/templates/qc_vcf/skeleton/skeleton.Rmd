...
all_chrosomoses <- lapply(
  ...
  FUN = function(ichr) {
    fread(
      ...
      rename(CHR = `#CHROM`) %>% 
      separate(col = "INFO", into = c("RefPanelAF", "AC", "AN", "INFO"), sep = ";") %>% 
      mutate_at(.vars = vars(c("RefPanelAF", "AC", "AN", "INFO")), .funs = ~as.numeric(gsub(".*=", "", .x))) %>% 
      mutate(
        # CHR = as.character(CHR),
        AF = AC / AN,
        bin_af = cut(AF, breaks = seq(0, 1, 0.1), include.lowest = TRUE),
        bin_info = cut(INFO, breaks = seq(0, 1, 0.1), include.lowest = TRUE),
        bin_af = fct_explicit_na(bin_af),
        bin_info = fct_explicit_na(bin_info)
      )
  }
)
...
all_chrosomoses %>% 
  ...
  count(bin_af) %>% 
  mutate(p = n / sum(n)) %>% 
  add_row(bin_af = "Total", n = sum(.$n), p = 1) %>%
  mutate(
    n = comma(n),
    p = percent(p, accuracy = 0.0001)
  ) %>% 
  rename(
...
all_chrosomoses %>% 
  ...
  count(bin_info) %>% 
  mutate(p = n / sum(n)) %>% 
  add_row(bin_info = "Total", n = sum(.$n), p = 1) %>%
  mutate(
    n = comma(n),
    p = percent(p, accuracy = 0.0001)
  ) %>% 
...
for (ichr in c(1:22, "X")) {
  ...
  p1 <- ggplot(
    data = tmp, 
    mapping = aes(x = RefPanelAF, y = AF, colour = INFO > 0.8)
  ) +
    geom_point(size = 0.1, na.rm = TRUE) +
    scale_colour_viridis_d(begin = 0.2, end = 0.8, na.value = "firebrick2") +
    labs(x = "HRC alternate allele frequency", y = "Alternate allele frequency") +
    scale_x_continuous(labels = percent_format(accuracy = 0.01), expand = c(0, 0)) +
    scale_y_continuous(labels = percent_format(accuracy = 0.01), expand = c(0, 0))
  print(p1)
  cat("\n\n")
  ...
  p2 <- ggplot(
    data = tmp %>% 
      count(bin_af) %>% 
      mutate(p = percent(n / sum(n), accuracy = 0.01)), 
    mapping = aes(x = bin_af, y = n, label = p)
  ) +
    geom_bar(fill = viridis_pal(begin = 0.5, end = 0.5)(1), stat = "identity") +
    geom_text(vjust = -0.1) +
    scale_y_continuous(
      labels = comma, 
  ...
  p3 <- ggplot(
    data = tmp %>% 
      count(bin_info) %>% 
      mutate(p = percent(n / sum(n), accuracy = 0.01)), 
    mapping = aes(x = bin_info, y = n, label = p)
  ) +
    geom_bar(fill = viridis_pal(begin = 0.5, end = 0.5)(1), stat = "identity") +
    geom_text(vjust = -0.1) +
    scale_y_continuous(
  ...
  p4 <- ggplot(
    data = drop_na(tmp, INFO), 
    mapping = aes(x = POS / 1e6, y = INFO, colour = INFO > 0.8)
  ) +
    geom_point(size = 0.1, na.rm = TRUE) +
    scale_colour_viridis_d(begin = 0.2, end = 0.8) +
    scale_x_continuous(labels = comma) +
    scale_y_continuous(expand = c(0, 0)) +
    labs(x = "Position (Mb)", y = "INFO score")
  print(p4)
  cat("\n\n")
  ...
  p5 <- ggplot(
    data = drop_na(tmp, INFO),
    mapping = aes(x = POS / 1e6, y = seq_along(POS) / 1e3)
  ) +
    geom_point(
      size = 0.1, 
    ...
    labs(x = "Position on chromosome (Mb)", y = "Line number (x 1,000)")
  print(p5)
  cat("\n\n")
}
```
