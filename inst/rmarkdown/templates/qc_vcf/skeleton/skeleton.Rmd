...
### Define theme
ggplot2::theme_set(ggplot2::theme_light(base_size = 11))
...
all_chrosomoses <- lapply(
  ...
  FUN = function(ichr) {
    ...
    out[, AF := AC / AN]
    out[, bin_af := forcats::fct_explicit_na(cut(AF, breaks = seq(0, 1, 0.1), include.lowest = TRUE))]
    out[, bin_info := forcats::fct_explicit_na(cut(INFO, breaks = seq(0, 1, 0.1), include.lowest = TRUE))]
  }
)
...
for (ichr in intersect(c(1:22, "X"), names(all_chrosomoses))) {
  ...
  p1 <- ggplot2::ggplot(
    ...
    mapping = ggplot2::aes(x = POS, y = seq_along(POS))
  ) +
    ggplot2::geom_point(
      size = 0.1, 
  ...
  p3 <- ggplot2::ggplot(
    data = tidyr::drop_na(tmp, INFO), 
    mapping = ggplot2::aes(x = POS, y = INFO, colour = INFO > 0.8)
  ) +
    ggplot2::geom_point(size = 0.1, na.rm = TRUE) +
  ...
  design <- "
    ...
    55
  "
  p_all <- patchwork::wrap_plots(list(p1, p2, p3, p4, p5), design = design) +
