...
fam_data <- data.table::fread(file.path(temp, "data.fam"))
fam_data[, (1:2) := lapply(.SD, as.character), .SDcols = 1:2]
data.table::setnames(
  x = fam_data, 
  old = sprintf("V%d", 1:ncol(fam_data)), 
  new = c("FID", "IID", "father_id", "mother_id", "sex", "phenotype")
)

samples_duplicated <- anyDuplicated(gsub("[_-][^-_]*$", "", fam_data[["IID"]]))
...
fam_data[, .N, by = c("cohort", "sex_fct")][, .(sex_fct, N)] %>% 
...
)[, labs := ifelse(F_MISS > 1 - params[["callrate_samples"]], IID, as.character(NA))][order(F_MISS)]
...
callrate_samples_geq <- data.table::fread(file.path(temp, "data_maf_geq.imiss"))[order(F_MISS)]
...
if (is_maf_lower) {
  ...
  callrate_samples_lower <- data.table::fread(file.path(temp, "data_maf_lower.imiss"))[order(F_MISS)]
...
ggplot2::ggplot(
  ...
  ggplot2::scale_y_continuous(
    breaks = (function(n = 5, cr_thresh = params[["callrate_samples"]], ...) {
      function(x) {
        x <- x[is.finite(x)]
        if (length(x) == 0) {
          return(numeric())
        ...
        rng <- range(x)
        c(labeling::extended(rng[1], rng[2], n, ...), cr_thresh)
      }
    })(),
    labels = (function(x, cr_thresh = params[["callrate_samples"]]) {
      ifelse(x == cr_thresh, paste0("<b style = 'color:firebrick2;'>", percent1(x), "</b>"), percent1(x))
    })
  ) +
...
sexcheck[, STATUS := ifelse(sex_discrepancy | is.na(sex_discrepancy) | `F` >= min(params[["sex_threshold"]]) & `F` < max(params[["sex_threshold"]]), "PROBLEM", STATUS)]
sexcheck[, labs := ifelse(STATUS == "PROBLEM" & F_MISS < 1 - params[["callrate_samples"]], IID, NA)]
...
ggplot2::ggplot(
  ...
  ggplot2::labs(
    x = "Homozygosity Rate",
    y = "Call Rate",
    title = "Homozygosity Rate Based on X-chromosomal Variants",
    subtitle = paste(
...
if (is_maf_lower) {
  ...
  patchwork::plot_annotation(
    ...
    tag_levels = "A", 
    subtitle = paste0(
      "The horizontal red lines (\"<b style = 'color:firebrick2;'>---</b>\") denote the ", params[["heterozygosity_threshold"]], 
...
if (!imendel_file_check) {
  ...
  mendel_errors[, labs := ifelse(percentage > params[["mendelian_samples"]], IID, NA)]
  mendel_errors[, labs_fam := ifelse(percentage > params[["mendelian_samples"]], FID, NA)]
  
  mendel_errors_geq <- data.table::fread(file.path(temp, "data_maf_geq.imendel"))[, percentage := N / snps_number]
  ...
  if (is_maf_lower) {
    mendel_errors_lower <- data.table::fread(file.path(temp, "data_maf_lower.imendel"))[, percentage := N / snps_number]
...
ggplot2::ggplot(
  ...
  ggplot2::scale_x_continuous(labels = comma, trans = "log10") +
  ggplot2::scale_y_continuous(
    breaks = (function(n = 5, me_thresh = params[["mendelian_samples"]], ...) {
      function(x) {
        x <- x[is.finite(x)]
        if (length(x) == 0) {
          return(numeric())
        ...
        rng <- range(x)
        c(labeling::extended(rng[1], rng[2], n, ...), me_thresh)
      }
    })(),
    labels = (function(x, me_thresh = params[["mendelian_samples"]]) {
      ifelse(x == me_thresh, paste0("<b style = 'color:firebrick2;'>", percent1(x), "</b>"), percent1(x))
    })
  ) +
  ggplot2::labs(
    x = "Number of Samples", 
...
if (file.exists(paste0(out, "-merge.missnp"))) {
  parallel::mclapply(1:22, mc.cores = 22, function(i) {
    sys(paste(plink, 
      "--bfile", paste0(params[["ref1kg_genotypes"]], "Chr", i),
...
data_pca <- rbind(
  ...
  )[, POP := params[["cohort_name"]]][, .(IID = V2, POP)],
  data.table::fread(
...
)[IID %in% as.character(data.table::fread(file.path(temp, "data_1kgenome.fam"))[["V2"]])]
data_pca <- merge(x = data_pca, y = data_1kg_eigenvec, by = "IID", all.x = TRUE)[, 
  POP := factor(
    x = POP, 
    levels = c(params[["cohort_name"]], sort(setdiff(unique(POP), params[["cohort_name"]]), decreasing = TRUE))
  )
][, .(IID, Population = POP, x = PC1, y = PC2)]
data_pca <- merge(
  x = data_pca,
...
centre_pca <- data_pca[Population != params[["cohort_name"]], .SD, .SDcols = c("IID", "Population", grep("centroid", names(data_pca), value = TRUE))][, (c("IID", "Population")) := lapply(.SD, as.character), .SDcols = c("IID", "Population")]
...
data_pca_plot <- merge(
  x = data_pca,
  y = merge(x = check_ethnicity[, .(IID, closest_pop)], y = fam_data[, .(IID, cohort)], all.x = TRUE, by = "IID"),
  all.x = TRUE,
  by = "IID"
)[, Population := as.character(Population)]
data_pca_plot[, cohort := ifelse(is.na(cohort), "1,000 Genomes", cohort)]
data_pca_plot[, closest_pop := ifelse(is.na(closest_pop), Population, closest_pop)]
data_pca_plot[, Population := ifelse(Population %in% params[["cohort_name"]], cohort, Population)]
data_pca_plot <- merge(x = data_pca_plot, y = pop1kg_description, all = TRUE, by.x = "Population", by.y = "POP")
data_pca_plot[, SUPER_POP := ifelse(is.na(SUPER_POP), Population, SUPER_POP)]
data_pca_plot[, SUPER_POP_NAME := ifelse(is.na(SUPER_POP_NAME), Population, SUPER_POP_NAME)]
data_pca_plot <- data_pca_plot[!is.na(cohort), ]
...
cat(
  ...
  ifelse(nrow(not_target_population) > 1, "s were", " was"),
...
data.table::setnames(eigenvec_pca, names(eigenvec_pca), c("FID", "IID", paste0("PC", seq_len(params[["pca_components"]]))))
eigenvec_pca[,
...
p_within[[length(p_within) + 1]] <- ggplot2::ggplot(
  data = data.table::fread(file.path(temp, "data_pop_within.eigenval"))[, y := V1 / sum(V1)][1:params[["pca_components"]]][, x := 1:.N],
  mapping = ggplot2::aes(x = x, y = y)
) +
  ggplot2::geom_col(fill = scales::viridis_pal(begin = 0.5, end = 0.5)(1)) +
  ggplot2::scale_x_continuous(
...
samples_to_exclude_export[, Status := ifelse((Sample_Call_Rate + Heterozygosity_Check) > 0, "Exclude", "Check")]
name_cols <- c(
  "FID", "IID", "Status",
...
snps_call_rate <- callrate_snps_raw[, F_MISS := ifelse(is.na(F_MISS), 1, F_MISS)][F_MISS >= 0.001][order(1 - F_MISS)]
p <- ggplot2::ggplot(
  data = snps_call_rate,
  ...
  ggplot2::scale_y_continuous(
    breaks = (function(n = 5, cr_thresh = params[["callrate_snps"]], ...) {
      function(x) {
        x <- x[is.finite(x)]
        if (length(x) == 0) {
          return(numeric())
    ...
    labels = (function(x, cr_thresh = params[["callrate_snps"]]) {
      ifelse(x == cr_thresh, paste0("<b style = 'color:firebrick2;'>", percent1(x), "</b>"), percent1(x))
    })
  ) +
  ggplot2::labs(
...
if (lmendel_file_check) {
  uniq_snps_vec <- setdiff(uniq_snps[["SNP"]], mendel_error_snps[percentage > params[["mendelian_snp"]], SNP])
} else {
...
sys(paste("cd", temp, "&& bash Run-plink.sh"))

data.table::fwrite(
  x = data.table::data.table(sprintf(paste0(imputation_filename, "-updated-chr%d"), 2:23)),
...
sys(paste(params[["bin_path"]][["bgzip"]], 
  "-c", paste0(imputation_filename, ".vcf"),
  ">", paste0(imputation_filename, ".vcf.gz")
))
sys(paste(params[["bin_path"]][["bcftools"]], "index", paste0(imputation_filename, ".vcf.gz")))
...
if (is.null(params[["ref1kg_fasta"]])) {
  sys(paste(params[["bin_path"]][["bcftools"]], 
    "annotate", 
...
`r if (params[["includes_relatives"]] & lmendel_file_check) cat("* Variants with a Mendelian error rate below", params[["mendelian_snp"]], "were removed.")`
...
output_xlsx[["exclude_flag"]] <- merge(
  x = samples_to_exclude_export,
...
data.table::setcolorder(
  x = output_xlsx[["exclude_flag"]], 
  neworder = c("FID", "IID", "Sex", "Cohort", "Status", setdiff(names(output_xlsx[["exclude_flag"]]), c("FID", "IID", "Sex", "Cohort", "Status")))
)
cols_order <- setdiff(names(output_xlsx[["exclude_flag"]]), c("FID", "IID", "Sex", "Cohort"))
output_xlsx[["exclude_flag"]] <- output_xlsx[["exclude_flag"]][order(-cols_order)]
